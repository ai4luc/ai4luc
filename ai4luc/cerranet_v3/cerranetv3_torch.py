...
class Cerranetv3(nn.Module):

    def __init__(self, num_classes=4):
        super(Cerranetv3, self).__init__()
...
def cerranet(pretrained=False, progress=True, **kwargs):
    r"""Cerranetv3 model architecture from the
    `"Cerranetv3: a deep convolutional neural network for classifying land use and land cover on Cerrado biome tocantinense" <https://drive.google.com/file/d/1JnN52C8yZKwN-5XA6qSiCsCygh1-0vvZ/view>`_ paper.

    Args:
        pretrained (bool): If True, returns a model pre-trained on Sports1M
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = Cerranetv3(**kwargs)
    if pretrained:
